...
IMG ?= ghcr.io/radiant-maxar/cluster-api-provider-vcluster:$(TAG)
...
# revert the values back to development ones 
	sed -i'' -e 's@image: .*@image: ghcr.io/radiant-maxar/cluster-api-provider-vcluster:latest@' ./config/default/manager_image_patch.yaml
