...
func (r *VClusterReconciler) Reconcile(ctx context.Context, req ctrl.Request) (_ ctrl.Result, reterr error) {
	r.Log.Info("reconcile", req.NamespacedName)
	...
	if err != nil {
		r.Log.Info(fmt.Sprintf("error during virtual cluster deploy %s/%s", vCluster.Namespace, vCluster.Name), err)
	...
	if err != nil {
		r.Log.Info(fmt.Sprintf("vcluster %s/%s is not ready", vCluster.Namespace, vCluster.Name), err)
	...
	if err != nil || !vCluster.Status.Ready {
		r.Log.Info("readiness check failed", err)
		return ctrl.Result{RequeueAfter: time.Second * 5}, nil
	}
...
func (r *VClusterReconciler) redeployIfNeeded(ctx context.Context, vCluster *v1alpha1.VCluster) error {
	...
	r.Log.Info("upgrade virtual cluster helm chart", fmt.Sprintf("%s/%s", vCluster.Namespace, vCluster.Name))
	...
	if vCluster.Spec.KubernetesVersion != nil && *vCluster.Spec.KubernetesVersion != "" {
		...
		if len(v) == 2 {
			...
			kVersion.Major = v[0]
			kVersion.Minor = v[1]
			r.Log.Info(fmt.Sprintf("vclusters %s/%s patch version defined in .spec.kubernetesVersion field will be ignored, latest supported patch version will be used", vCluster.Namespace, vCluster.Name))
		} else {
	...
	r.Log.Info(fmt.Sprintf("Deploy virtual cluster %s/%s with values", vCluster.Namespace, vCluster.Name), values)
...
func (r *VClusterReconciler) checkReadyz(vCluster *v1alpha1.VCluster, restConfig *rest.Config) (bool, error) {
	...
	r.Log.Info(fmt.Sprintf("%s/%s: ready check took", vCluster.Namespace, vCluster.Name), time.Since(t))
	if err != nil {
		return false, err
	}
...
func (r *VClusterReconciler) deleteHelmChart(ctx context.Context, namespace, name string) error {
	...
	r.Log.Info("delete vcluster helm release", fmt.Sprintf("%s/%s", namespace, name))
